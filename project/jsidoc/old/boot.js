 ...
 */
function J(){this.onload=Function.prototype;var A=document.getElementById("content").contentWindow;if(M)clearInterval(M);M=setInterval(function(){var _=decodeURIComponent(O.hash.substr(1)),$=A.location;if(_&&_!=$.href)$.replace(_)},100);if(JSIDoc.waitExternalScript){var _=["加载本机文档数据失败:","请直接输入文档源代码!!"].join("\n"),B=prompt(_);if(B)window.eval(B)}var $=JSIDoc.packageGroupMap;if($.length==0){$.push("托管脚本示例");$.托管脚本示例=["example","example.alias","example.internal","example.dependence"]}JSIDoc.addPackageMap($);setTimeout(function(){document.getElementById("menu").setAttribute("src","html/controller.html?@menu")},1)}var JSIDoc={prepare:function(){window.onload=J;var A=window.location.search,$=this.packageGroupMap=[];if(A&&A.length>2){var C=/([^\?=&]*)=([^=&]*)/g,_;while(_=C.exec(A)){var B=decodeURIComponent(_[1]),D=decodeURIComponent(_[2]);


if("externalScript"==B){

if(window.clipboardData&&D.indexOf("file://")==0){
var E=window.clipboardData.getData("Text");

window.eval(E)

}else{this.waitExternalScript=true;document.write("<script src='"+D+"'><\/script>")}return D}else if(D)if(B=="group"){D=JSON.decode(D);for(B in D){$.push(B);$[B]=D[B]}}else if(B=B.replace(/^group\.(.+)|.+/,"$1")){$.push(B);$[B]=D.split(",")}}}},addPackageMap:function(H,E){this.waitExternalScript=false;this.rootInfo=C.requireRoot();this.packageInfoGroupMap=this.packageInfoGroupMap||[];this.packageInfoMap=this.packageInfoMap||{};for(var D=0;D<H.length;D++){var B=H[D],G=H[B],G=A(G,E),I=this.packageInfoGroupMap[B];if(!I){this.packageInfoGroupMap[B]=I=[];this.packageInfoGroupMap.push(B)}$:for(var $=0;$<G.length;$++){var F=C.require(G[$]),_=I.length;while(_--)if(I[_]==F)continue $;I.push(F);this.packageInfoMap[F.name]=F}}},collapsePackage:function(_){var $=document.getElementById(I).contentWindow.document;B.loadPackage($,_)},render:function(C){var D=C.location.href;D=D.replace(/^([^#\?]+[#\?])([^#]+)(#.*)?$/g,"$2");if(D=="@menu")C.write(this.genMenu());else{var $="#"+encodeURIComponent(C.location.href);O.hash=$;if(D=="@export")C.write(this.genExport(C));else{var _=D.lastIndexOf("/");if(_>=0){var E=D.substr(0,_).replace(/\//g,"."),B=D.substr(_+1);C.write(this.genSource(E,B))}else{var A=D.split(":");if(A[1])C.write(this.genObject(A[0],A[1]));else this.genPackage(A[0],C)}}}},genMenu:function(){var _=$("menu.xhtml"),A=_.render({JSIDoc:JSIDoc,rootInfo:JSIDoc.rootInfo,packageInfoGroupMap:JSIDoc.packageInfoGroupMap});return A},genExport:function(_){var B=$("export.xhtml"),C=[];for(var A in this.packageInfoMap)C.push(A);var D=B.render({packageNodes:ExportUI.prepare(_,C)});return D},genPackage:function(B,G){var E=$("package.xhtml"),D=C.require(B),F=D.getInitializers();F.push(function(){var F=D.getObjectInfos(),A=[],C=[],_=[];for(var H=0;H<F.length;H++){var B=F[H];switch(B.type){case"constructor":A.push(B);break;case"function":C.push(B);break;default:_.push(B)}}var $=E.render({constructors:A,functions:C,objects:_,files:D.fileInfos,packageInfo:D});G.open();G.write($);G.close()});var A=0;function _(){if(A<F.length){F[A++]();_()}}_()},genObject:function(_,B){var D=C.require(_),E=D.getObjectInfoMap()[B];switch(E.type){case"constructor":var A=$("constructor.xhtml");break;case"function":A=$("function.xhtml");break;case"object":A=$("object.xhtml");break;default:A=$("native.xhtml")}return A.render({objectInfo:E})},genSource:function(A,B){var G=_.require(B,A),H=G.source.toString(),D=G.docEntries,E=[];if(D!=null&&D[0]!=null)for(var J=0;J<D.length;J++){var I=D[J].getId();if(I&&I.length>0)E.push({name:I,position:D[J].end})}G.anchors=E;var C=G.parse(),F=$("source.xhtml");F.beforeOutput=function(){};return F.render({JSIDoc:JSIDoc,lines:C})},


cacheScript:function(A,B){
this.waitExternalScript=false;
B=B||"未命名分组";
var _=this.packageGroupMap[B];
if(!_){
_=this.packageGroupMap[B]=[];
this.packageGroupMap.push(B)
}
for(var $ in A){
	if($){
		_.push($);
